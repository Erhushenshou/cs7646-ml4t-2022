...
def compute_portvals(  		  	   		  	  			  		 			     			  	 
    ...
    orderdata = pd.read_csv(orders_file, index_col='Date')
    ...
    for i in range(orderdata.shape[0]):
        ...
        if option == 'BUY':
            ...
            holdingtable.loc[purchasedate, 'cash'] = holdingtable.loc[purchasedate, 'cash'] - shares * price * (1 + impact) - commission
    ...
    holdingtable.loc[:, 'portval'] = (holdingtable.iloc[:, range(0, buyingtable.shape[1])] * buyingtable).sum(axis=1) + holdingtable.loc[:, 'cash']

    #print(holdingtable.iloc[:, range(0, buyingtable.shape[1])])

    #print(holdingtable)
    #print(buyingtable)
    df_trades = holdingtable.loc[:, 'portval']
    #print(portvals)
